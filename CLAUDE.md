...
- **No node-tree diffing anywhere.** Every render does `container.innerHTML = ""` and rebuilds the whole SVG from scratch. For maps of a few dozen nodes this is fast enough not to matter, and it eliminates an entire class of "stale DOM node" bugs — see the "Bugs worth remembering" section below for what happens when you build a node detached and measure it before attaching. It stops being "imperceptibly fast" well before that, though: a full rebuild measured ~48ms typical (up to ~180ms) on a 79-node map, blowing the 16.7ms frame budget — see `applyCamera()` in `render.ts` for the one place this mattered enough to special-case (continuous zoom).
